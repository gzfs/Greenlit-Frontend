...
import { MaterialSymbolsLightArrowOutward } from "./Icons";
...
export default function Onboarding() {
  ...
  const [currentSPAPage, setCurrentSPAPage] = useState("Dashboard");
  ...
  // Update the sidebar callback
  const handlePageChange = (page: string) => {
    if (page === "Dashboard" && progress === 100) {
      setCurrentView("dashboard");
    } else {
    ...
    <main className="p-8">
      ...
      <AnimatePresence mode="wait">
        {currentView === "dashboard" ? (
          ...
          <motion.div
            key="questionnaire"
            className="col-span-9 w-full place-self-center relative"
          >
            <AnimatePresence mode="wait">
              {showAnalytics ? (
                <motion.div
                  ...
                  className="p-8"
                >
                  <h2 className="text-2xl font-semibold text-emerald-700 mb-8">
                    ESG Analytics Dashboard
                  </h2>
                  <DashboardVisualizations data={mockVisualizationData} />
                </motion.div>
              ) : selectedBox ? (
                <motion.div
                  key="enlarged"
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="absolute -translate-x-1/2 left-1/2 top-1/2 filter backdrop-blur-md w-[600px]"
                >
                  <motion.div
                    layout
